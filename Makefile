...
# Build target for darwin arm
build-darwin-arm: $(addsuffix .darwin-arm,$(BUILD_TARGETS))

.PHONY: build-linux-arm
# Build target for linux arm
build-linux-arm: $(addsuffix .linux-arm,$(BUILD_TARGETS))
...
# Trigger the build operation for darwin-arm. Notice that the suffix is removed as it is only used for Makefile expansion purposes.
%.darwin-arm:
	@echo "Building darwin arm binary $@"
	@GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 $(GO_BUILD) $(GO_LDFLAGS) -o $(BIN_FOLDER)/darwin-arm/$(basename $@) ./cmd/$(basename $@)/main.go

# Trigger the build operation for linux-arm. Notice that the suffix is removed as it is only used for Makefile expansion purposes.
%.linux-arm:
	@ echo "Building linux arm binary $@"
	@GOOS=linux GOARCH=arm64 CGO_ENABLED=0 $(GO_BUILD) $(GO_LDFLAGS) -o $(BIN_FOLDER)/linux-arm/$(basename $@) ./cmd/$(basename $@)/main.go
...
release: clean build-darwin build-linux build-darwin-arm build-linux-arm k8s
	@mkdir -p $(BUILD_FOLDER)
